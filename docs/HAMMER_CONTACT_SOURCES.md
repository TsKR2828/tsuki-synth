...
## 5. 遲滯（hysteresis）：文獻有模型，本文件未取得參數

真實槌氈在壓縮與回彈時走不同的曲線（能量耗散）。Stulov 提出了含遲滯的
氈模型，是 `F = K·δ^α` 的推廣。**本文件沒有取得該模型的參數值**，
若要納入需另行溯源。純冪律（無遲滯）是文獻中常用的簡化，可作第一階段。
...
| 遲滯參數 | ❌ 未取得（§5） |
| 非鋼琴引擎（beam/plate）的 `K`、`α` | ❌ 完全沒有文獻。舌鼓/鑼的橡膠槌、木槌無對應量測 |

**最後一列是真正的風險**：這批資料是**鋼琴專屬**的。Cimbalom/Piano 路徑
可以直接受惠，Chromatic（tongue drum / water gong）沒有對應數據，
硬套等於把鋼琴槌氈的常數搬到別的樂器上——那會違反 Rule 4。
...
- [ ] 遲滯參數未取得（§5）
- [ ] beam／plate 引擎無對應文獻（§6，**這是本項最大的阻擋**）
