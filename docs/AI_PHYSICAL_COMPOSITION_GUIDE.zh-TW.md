...
> 版本：2.1（同步 2026-07-17 深度修正：嚴格 score、邊界條件、geometry mode）
> 日期：2026-07-17（§1–§12 原文 2026-06-21）
> 適用格式：`TsukiSynth Score v1`

> **第一原則（2026-08-28 月月市場情報，見 `docs/PRODUCT_MARKET_NOTES.zh-TW.md` §4）：
> 產出單位＝完整樂曲（至少完整樂章），絕不是孤立音效——10 秒兩聲鐘響賣不掉，
> 完整改編賣得掉。** 任何作曲/產線工作在動手前都先確認這一條。
...
梁模型適合短促、清脆、非整數泛音的音色。`tongue_drum`／`beam` 預設是
`beam_boundary: "cantilever"`（舌片一端固支、一端自由）；只有明確寫
`"free_free"` 才使用懸掛自由梁。兩種邊界使用各自的解析特徵根與振型：
...
> **重要：** 預設 `frequency_mode: "midi"` 會先由尺寸／材料算自然模態，再把 f0
> 對齊 score 的 `note`；尺寸仍影響泛音比例、模態質量振幅與衰減。若明確使用
> `frequency_mode: "geometry"`，則保留物理計算出的絕對頻率，不對齊 MIDI。
...
tongue_drum（預設 fixed-free cantilever 梁）與 water_gong（圓板）的泛音比例
**不是**整數倍數：

| 引擎 | 前 4 個部分音比例（f_n / f_1） |
|---|---|
| tongue_drum（cantilever） | 1.000 : 6.267 : 17.548 : 34.386 |
...
以下是 2026-07-17 重新產生的 **MIDI 60 參考 probe**；只列落在 12-TET 音程
±10 cents 內的局部極小值。它們會隨材質、敲擊位置、邊界與自訂 ratios 改變，不能
取代實際 score 的模態檢查。完整曲線與 probe 參數見 `reports/consonance_tables.md`。

| 引擎（自身音程） | 可用半音音程 | 對應局部極小值 (cents) | dissonance 值 |
|---|---|---:|---:|
| cimbalom | 大三度 / 純四度 / 三全音 / 純五度 / 大六度 / 八度 | 390 / 505 / 590 / 705 / 890 / 1200 | 0.2934 / 0.1911 / 0.2255 / 0.1222 / 0.1789 / 0.0721 |
| tongue_drum（cantilever） | 除同度外，沒有落在 ±10c 的 12-TET 局部極小值 | 870 / 955 / 1165 均離最近半音 >10c | 不可把舊 free-free 的 695c 規則沿用 |
| water_gong（clamped probe） | 小三度 / 八度 | 305 / 1200 | 0.3053 / 0.2580 |
| water_gong_free | 純四度 / 大七度 | 505 / 1090 | 0.0224 / 0.0817 |
| piano（modal） | 純四度 / 三全音 / 純五度 / 八度 | 500 / 590 / 705 / 1200 | 0.0770 / 0.0813 / 0.0404 / 0.0144 |

**明確不建議的音程**要讀同一次產生的局部極大值，不能用一條規則涵蓋所有音色。
目前 cimbalom / clamped water_gong / piano 的第一個高粗糙度區約在 55–95c；
tongue_drum 與 free plate 的曲線不同。這是模型排序，不是「必然難聽」的主觀宣判。

**反直覺的重點**：cimbalom 的大三度（400c，dissonance 0.2996）比它自己的增四度
（590c 局部極小值 0.2255）更粗糙，兩者又都高於純五度（705c，0.1222）；數值來自指定 probe 的
Sethares roughness（弦的伸張非諧性與振幅分布共同造成），
不是筆誤。**要用哪個音程，先查表設計，再以實曲 event-specific checker 驗收，
不要套用鋼琴和聲的直覺。**
...
|---|---|---:|---:|
| cimbalom | tongue_drum | 200 / 700 / 1200 | 0.0805 / 0.0125 / 0.0021（極小值 200/710/1195c） |
| cimbalom | water_gong（clamped probe） | 300 / 600 / 1200 | 0.2662 / 0.2946 / 0.1373（極小值 290/595/1195c） |
| tongue_drum | water_gong（clamped probe） | 100 / 400 / 1200 | 0.0814 / 0.0421 / 0.0620（極小值 90/395/1200c） |

舊 free-free beam 表曾得到 tongue_drum→water_gong 的 1000c 結論；cantilever 修正後
已不成立，不能沿用。新 clamped reference 的可達 12-TET minima 是 100/400/1200c。
這也說明表格必須跟模型版本一起重生，不能把歷史數字寫成永恆音樂規則。

**重要限制**：上表只是固定 MIDI 60 標準 probe 的設計參考，不是成品驗收。
`tools/check_piece_consonance.py` 會按實際高低音排序，從 score 的 C++
`--dump-modes` 讀取每個 event 的所有弦與模態，再以其材質、尺寸、邊界、敲擊點和
速度相依槌頻譜重算曲線；因此不再有固定方向白名單。反過來說，只查上表而沒有跑
event-specific checker，不能宣稱該曲音程已驗證。
...
`check_piece_consonance.py` 會列出每一對「宣告時間重疊」的音符，以該兩個 event
的真實模態頻譜重建 Sethares 曲線，判定實際音程是否離最近局部極小值不超過
10 cents；PASS/VIOLATION/UNVERIFIED 逐條列出並寫入報告。它採 interval class
八度等價，且不推算宣告 duration 之後的共鳴尾巴；FM 與 Custom Harmonics 明列
`UNVERIFIED-DOMAIN`。**不是自己聽過覺得順耳就交差。**
...
（純五度、八度附近有明確局部谷），適合擔任「和聲」；genuinely 非諧的引擎
（tongue_drum、water_gong）部分音分散。不同引擎的譜已各自正規化，**dissonance
絕對值不可直接跨引擎縱向排名**；只能比較同一條曲線內的相對高低。這些音色
...
| 角色 | 適合引擎 | 理由（來自表格的數字） |
|---|---|---|
| 和聲（同時多聲部、和弦、和聲進行） | cimbalom、piano | 部分音接近整數比，純五度／八度附近有穩定局部 minima；仍須用當次 probe 數字 |
| 色彩/節奏（單旋律點綴、標記、脈衝） | tongue_drum、water_gong、water_gong_free | 非整數部分音分散；reference 曲線與傳統和聲直覺不同，不用跨引擎絕對值排名 |
| 跨引擎同時發聲 | 先用 §13.3 參考表設計，再跑 event-specific checker | 固定 probe 不能涵蓋實曲的材質、幾何、音域與演奏參數 |
...
最新獨立窄頻實測中，可量測 probe 的 measured/model 約 0.99–1.00；短到不足八個
週期者會明列 `UNVERIFIED/N/A`，不當作 PASS。`ModalResonator::excite()` 的
...
下表由 2026-07-17 最新模型重新產生，是工具固定 reference 材質／參數的快照。
T60 會受材質、`damping_override`、音高與模式影響，不可當所有 score 的固定常數；
新作品仍應以自己的 `--dump-modes` 為準。

| 引擎 | 音域 (MIDI) | T60 (s) | 最小重擊間距 = T60/3 (s) | 密度上限 (音符/秒) |
|---|---:|---:|---:|---:|
| cimbalom | 48 / 60 / 72 | 35.149 / 26.893 / 14.948 | 11.716 / 8.964 / 4.983 | 0.09 / 0.11 / 0.20 |
| tongue_drum | 48 / 60 / 72 | 36.858 / 30.120 / 18.673 | 12.286 / 10.040 / 6.224 | 0.08 / 0.10 / 0.16 |
| water_gong（clamped probe） | 48 / 60 / 72 | 8.064 / 7.488 / 6.024 | 2.688 / 2.496 / 2.008 | 0.37 / 0.40 / 0.50 |
| water_gong_free | 48 / 60 / 72 | 8.064 / 7.488 / 6.024 | 2.688 / 2.496 / 2.008 | 0.37 / 0.40 / 0.50 |
| piano | 48 / 60 / 72 | 35.149 / 26.893 / 14.948 | 11.716 / 8.964 / 4.983 | 0.09 / 0.11 / 0.20 |

（FM 不在表內——它是 ADSR 包絡，不是模態衰減，§0 驗證域聲明已標註 FM 域外。
表內 reference 與實際 score 參數不同時，數字也會不同。）

### 16.3 用法

寫曲時，任何「同一引擎、同一音高」的連續事件，起始時間差要 ≥ 該事件由
`--dump-modes` 得到的 T60/3；一段時間內同一音高的密度不要超過其倒數。不同音高
