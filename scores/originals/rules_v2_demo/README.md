...
> 2026-07-17：跟隨 cantilever、模態振幅與阻尼修正重新產生 reference tables；
> score 的每個 steel cimbalom event 明確寫入 `damping_override: 0.5`，避免材質資料庫
> 更新後暗中改變本示範所宣稱的 T60/3 時值契約。合規檢查器也已改成讀取本曲每個
> event 的真實 C++ `--dump-modes`，不再用固定 MIDI 60／固定引擎方向的參考表代替。
...
## 匯聚 Cell（依段落根音選用已驗證音程）

```
t0        : cimbalom 持續音 = section 根音（root）
t0        : A/A2/A3/尾聲的 tongue_drum = root+2 半音（M2，200 cents）
              -> 各 MIDI 55 event 的實際模態曲線局部極小值 200 cents（距離 0）
            B/B_echo 的 tongue_drum = root+7 半音（P5，700 cents）
              -> 各 MIDI 60 event 的實際模態曲線局部極小值 700/705 cents（距離 0/5）
t0+0.45s  : water_gong = root+12 半音（八度，1200 cents）
              -> 各 event 的實際模態曲線局部極小值 0/1200 cents（八度等價，距離 0）
              tongue_drum 的「宣告 duration」已於 t0+0.35s 結束；物理模態尾巴仍會
              重疊，現有 checker 不計這段——見下方範圍限制。
```

`tools/check_piece_consonance.py` 對本曲的驗證結果（`reports/rules_v2_demo_consonance_check.md`）：
**13 個同時發聲音程對，13 個 PASS，0 個違規、0 個 UNVERIFIED。**
...
- +7（700c）：實際局部極小值 705c，dissonance=0.1222
- +12（1200c，八度）：dissonance=0.0721
- +4（400c，M3）：較弱的局部極小值（390c，dissonance=0.2934）
...
本曲不是使用 material database 的 steel alpha，而是每個 cimbalom event 明確設定
`damping_override: 0.5`。最新 `--dump-modes` 在 MIDI 55 得 T60=1.9666s，
T60/3=0.6555s。本曲 cimbalom 琶音的 onset 間距固定
...
tongue_drum 與 water_gong 的同音重擊約每 17.75 秒一次（約 0.056/s），低於最新
reference table 的 tongue_drum 0.08–0.16/s 與 water_gong 0.37–0.50/s 上限。

## 已知範圍限制（誠實寫出，不是隱藏的簡化）

1. `tools/check_piece_consonance.py` 只檢查 score JSON 宣告的 `[time,
   time+duration)` 視窗有沒有重疊，不模擬敲擊後實際共鳴環繞超出宣告時長的部分。
   由於全曲在匯聚 cell 之外刻意保持宣告上的單旋律，這個
...
2. Cimbalom 自身的 T60（~2 秒）比 0.75 秒的琶音間距長很多，代表**同一句琶音裡
   ...
   完全沒有殘響交疊」的宣稱。
3. Checker 現在會用每個事件的材質、尺寸、邊界、敲擊點、速度相依槌頻譜與所有弦
   重新建立 event-specific 曲線，因此沒有固定方向白名單；但仍採「八度等價」的
   interval-class 簡化，且只把物理模態引擎判為域內。FM／Custom Harmonics 會明列
   `UNVERIFIED-DOMAIN`，不會被包裝成物理驗證通過。
...
python tools/check_piece_consonance.py scores/originals/rules_v2_demo/rules_v2_demo_001.score.json --out reports/rules_v2_demo_consonance_check.md
