...
## 00. 白話導讀（先讀這一頁就夠，2026-08-15 補）

> 月月 2026-08-15 回饋「看了但看不懂」，本節為此補上。以下不含專有名詞，
> 數字都能在 §3 表格對應回去。

### 這份報告在回答什麼

7/22 那一輪為了把物理改對，動了 6 個地方。這 6 個地方會讓**已經做好的曲子渲染出來不一樣**。
規則 10 要求：只要既有曲子會變，就要先告訴你變了多少，由你決定要不要留。這份報告就是那份告知。

**它不是在說哪裡壞掉了，是在說「改對之後的副作用有多大」。**

### 一句話結論

**8 首測試曲裡有 7 首變小聲、變亮、音尾變短；只有 `physical_piano` 反而變大聲。**

### 為什麼會這樣（最主要的一個原因）

程式裡每個音都有一個數字，代表「這個音要響多久」。
以前程式把這個數字**理解錯了**——它把「衰減到剩 37%」的時間，當成「衰減到聽不見」的時間用。
這次改成標準定義（衰減到聽不見 = -60 分貝）。

後果：**同一個數字，音尾長度變成原本的大約 1/6.9。**
音尾短了 → 整首曲子的總能量少了 → 量到的響度就變小。
7 首變小聲、時長略短，全部是同一個原因。

其餘 5 項改動（板的振型算法、槌子接觸時間、弱泛音存活判定、殘響換引擎、舌鼓邊界）
都是疊在這上面的次要影響。
...
| 曲子 | 變小聲多少 | 音色變亮多少 | 白話 |
|---|---|---|---|
| `fur_elise_opening`（FM） | −2.26 dB | 幾乎不變 | 這首只換了殘響引擎，用來當「純殘響影響有多大」的對照組 |
| `moonlight_sonata_i_tongue_drum` | −3.68 dB | 明顯變亮 | **泛音整組換掉**，音色差異最大的一首（見下） |
| `akashic_opening_bell_001` | −1.21 dB | 明顯變亮 | 同上，但曲子短、影響小 |
| `water_gong_clamped` | −1.65 dB | **大幅變亮** | 板的振幅算法改成真正的物理公式 |
| `water_gong_free` | −3.71 dB | 大幅變亮 | 同上，這首吃到最完整的改動 |
| `physical_piano` | **+2.35 dB（變大聲）** | 略變亮 | 唯一的例外，原因見下 |
| `moonlight_sonata_i_yangqin` | −2.52 dB | 幾乎不變 | 音高完全沒動 |
| `vivaldi_summer_m3` | −2.65 dB | 變亮 | 這首同時也改了曲子本身的殘響設定，兩個來源分不開 |

### 兩個值得你特別知道的點

1. **舌鼓那兩首的音色會明顯不一樣，不只是變小聲。**
   舌鼓的邊界預設從「兩端都懸空的鐵條」改成「一端固定的舌片」。真實的舌鼓就是後者，
   所以這是改對。但兩者的泛音比例表完全不同 → 每個音的泛音位置整組換了一套。
   **基頻（主要音高）沒變，變的是音色。**

2. **`physical_piano` 為什麼反而變大聲？**
   另一項改動（弱泛音存活判定）讓原本被提早砍掉的中高泛音活得更久。這首剛好只有 4 個
   短促重疊的音，這項的加成蓋過了音尾變短的減損。報告誠實標註：這是觀察到的方向，
   不是逐項拆解證明過的因果。

### 哪些數字**不要**看

§3 表格的「T60（寬帶估計）」那一欄，對音符多的完整樂曲**不可靠**
（`vivaldi_summer_m3` 算出 1259.8 秒，比整首曲子還長，是量測方法外推失真，不是真的響那麼久）。
只有 `water_gong_clamped` / `water_gong_free` 這種單音曲目的數字可以參考。

### 你需要決定的事

這 6 項全部是「把錯的定義改成標準定義」的方向，聲音變化是副作用、不是目的。
**選項只有兩個：整批接受（既有曲子的響度/音色就是會變成現在這樣），或指名某一項要回退。**
如果接受，既有 corpus 已經在新版本下全部重驗過 73/73 全過，不需要再改任何曲子。
