...
(2) 「先 commit+push+merge」（解除 R7）；(3) 「先試試看 A，記得開 Dynamic workflow」。

**UI 線**
- **雙開門提案被否決**（月月原話：「左側那麼寬了但旋鈕超小；右側一點也沒有鋼琴／揚琴／
  空靈鼓的視覺感，看上去像廉價玩具」）。裁定**撇開現行 UI 的所有既有元素**，
  由設計端從功能重新設計。`uiux/double_door_mockup.html` 與
  `docs/uiux/DOUBLE_DOOR_PROPOSAL.zh-TW.md` 作廢，只留歷史。
- 新的設計輸入 **`docs/uiux/UI_FUNCTIONAL_SPEC.zh-TW.md`**：只清點功能，
  刻意不寫顏色／字體／尺寸／佈局。內容從程式碼本體清點（60 個 APVTS 參數全表 +
  非參數控制項 + 6 條使用情境 + 8 條硬性約束 + §6 誠實揭露不存在的東西）。

**merge**
- 月月明示授權後，分支全數 merge → `main`（`64afb49`、`b7e4330`），
  `main` 從 B4 一路補到 B6 收官 + 驗證鏈補洞 + UI 裁決落地。`git diff main <branch>` 為空。

**影片工具**
- `tools/melody_roll_video.py` 換月月指定的霓虹配色（深紫底＋星點＋光暈，`--theme neon` 預設，
  `--theme slate` 保留原單色）＋**左側固定音名尺**（半音階梯 + 音名，密度依音域自動選）。
  偏差顯著性保留：FAIL 用遠離常態色相的紅橘 **＋ onset 三角形狀線索**。

**驗證線：裁決包選項 A（本輪最大進展）**
- 觸發：月月看完影片指出「紫色那麼多就沒有辦法驗證樂曲的正確性」——**指控成立**，
  給愛麗絲全曲 905 顆有 862 顆（95.2%）拒答。
- 裁決包 `reports/decision_packets/POLYPHONIC_VERIFICATION_OPTIONS.zh-TW.md`：
  A（逐事件分軌 + 疊加證明）／B（監督式 NMF）／C（神經網路轉譜）。
  月月選 A。關鍵有利事實：CLI 對每個 event **當場 new 獨立 voice** 再相加
  （ScoreRenderer.h L882/961/1053），所以分軌是**重放我們自己的計算**，不是估計。
- 以 Dynamic Workflow 發包（3 勘查 → 1 實作 → 1 實跑 → 2 Opus 稽核 → 1 修正）。
  **稽核抓到 blocker**：工兵把每軌峰值正規化（每顆音被墊高 +17~+76 dB），
  使 melody_verify 的絕對音量門檻失效，做出**假綠燈**（velocity 0.001、
  實際 −76 dBFS 的音被判 PASS）。已修，攻擊重放後正確 FAIL。
  另兩個變異（漏一軌不被抓、exit code 護欄空轉）也補上哨兵。測試 10 → 21。
- 產物：`tools/stem_verify.py` + `tests/test_stem_verify.py`（21 測試）。
  給愛麗絲全曲（乾聲）：**音高 883/905 顆有量測、全在 ±5 c 內（最大 2.4266 c）；
  起音 671/905 顆精測、全在 ±10 ms 內（最大 6.9272 ms）**；
  拒答 862 → 212。**疊加證明 ESTABLISHED**（殘差 −118.60 dBFS vs 門檻 −85 dBFS）
  ——「逐軌驗過」可在數學上轉移到成品。
- **新發現 1（工具層）**：`melody_verify` 的頻帶質心音高法會被**殘響的梳狀著色**拉偏，
  最大 ~9.5 cents，而容差只有 5 cents。同一顆 E4：帶殘響 −9.48 c FAIL／乾聲 −0.02 c PASS；
  獨立高解析度量測證明渲染端偏差僅 −0.03 c。**此缺陷本來就在現行 GATE 內**，
  以往密集段幾乎全拒答故未浮現。§7 v1 曾把同類現象歸因於「鄰近低音裙擺」，
  現在確認殘響著色是同一類機制的第二個來源。→ **主張域新規則：音高判定只在乾聲上有效。**
- **新發現 2（引擎層，待裁決 A14）**：22 顆高音基頻帶近乎無能量
  （G5 主導在第二 partial、基頻低 21.9 dB；G6 低 24.0 dB；D7 整體僅 −64.5 dBFS）。
  比值 > 2 與 stiffness inharmonicity 一致，真鋼琴高音區基頻本來就弱 → **未必是缺陷**。
- **月月獨立查核**（追加於 `reports/gate_outputs/stem_verify_fur_elise_run.txt`）：
  確認方法合理、21/21 通過；**裁定主張必須拆成音高／起音兩個維度**，
  不可壓成單一 verdict；**明令不可宣稱「泛音已驗證」**（partial 從未實測）；
  指出 ±5 cents 是本專案裁定門檻而非 ISO 標準，量測器須先自證 ≤1 cent；
  並更正原報告「起音全部 ±3 ms」的範圍描述（該數字只涵蓋帶殘響版 122 個 FAIL 中的 76 顆，
  全曲最大為 7.0522 ms／乾聲 6.9272 ms，仍全部通過 ±10 ms）。
- 主張域更新寫入 `docs/EARFREE_MELODY_GATE_DESIGN.zh-TW.md` §8（v2）。
- 新立卡：A13（partial GATE 主張域）、A14（高音弱基頻裁決）、
  C10（量測器合成哨兵，≤1 cent）、C11（逐顆拒答理由）、C12（`--analysis-dry` + hash/diff）、
  C13（harmonic-aware 判定）。

**未做／不主張**：partial 頻率與振幅從未實測；B/C 選項未動工；
外掛路徑 16 voice pool 的 voice stealing 疑慮（裁決包 §6）仍未量測。
