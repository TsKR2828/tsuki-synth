...
> 文件建立日期：2026-05-06
> 最後更新：2026-05-08
> 狀態：Phase 8 完成 — VST 插件功能基本完工，GUI 設計待月月參與
...
### Phase 0：規劃 ✅ 完成（2026-05-06）
- [x] 確認技術路線（JUCE + C++17 + CMake）
- [x] 評估三個引擎的移植難度
- [x] 撰寫 README + 本報告
- [x] 研究物理建模方向 + 開源資源盤點
- [x] 選定專案名稱與 repo（GitHub: `TsKR2828/tsuki-synth`）
- [x] 確認開發環境安裝清單

### Phase 1：環境建置 + 骨架 ✅ 完成（2026-05-08）
**目標：空殼插件能在 DAW 中載入、接收 MIDI、發出正弦波**

| 任務 | 說明 | 預估 | 實際 |
|------|------|------|------|
| 1.1 | 安裝 VS 2026 Community + CMake 4.3.2 | 0.5 天 | ✅ 已安裝（D:\Visual_Studio） |
| 1.2 | 建立 JUCE 專案骨架 (CMakeLists.txt) | 0.5 天 | ✅ JUCE 8 submodule + CMake |
| 1.3 | PluginProcessor: 接收 MIDI → 發正弦波 | 1 天 | ✅ 16 voice polyphonic sine synth |
| 1.4 | 確認 VST3 可在 Cubase 中載入 | 0.5 天 | ✅ Cubase LE AI Elements 12 驗證通過 |
| 1.5 | 設定 CI（可選，GitHub Actions） | 1 天 | ⏭️ 延後 |

**交付標準**：在 DAW 中掛載插件、按 MIDI 鍵盤能聽到聲音。 ✅ 達成

### Phase 2：低階 DSP 模組 + 材質數據庫 ✅ 完成（2026-05-08）
**目標：建立可重用的 DSP 底層元件，以及物理建模所需的材質資料**

| 模組 | 說明 | 檔案 | 狀態 |
|------|------|------|------|
| `Oscillator` | 相位累加器 + Sin/Saw/Square/Triangle | `src/dsp/Oscillator.h` | ✅ |
| `Envelope` | ADSR + ExpDecay per-mode damping | `src/dsp/Envelope.h` | ✅ |
| `BiquadFilter` | IIR biquad (LP/HP/BP/Notch) | `src/dsp/BiquadFilter.h` | ✅ |
| `DelayLine` | Circular buffer + 線性插值 + feedback | `src/dsp/DelayLine.h` | ✅ |
| `NoiseGen` | White + Pink (Paul Kellet IIR) | `src/dsp/NoiseGen.h` | ✅ |
| `LFO` | 低頻振盪器（複用 Oscillator） | `src/dsp/LFO.h` | ✅ |
| `ModalResonator` | **核心：N 模態衰減正弦波渲染器** | `src/dsp/ModalResonator.h` | ✅ |
| `MaterialDB` | 載入 materials.json 查詢材質 | `src/physics/MaterialDB.h` | ✅ |
| `StringModel` | 弦振動模態計算（頻率/衰減/振幅） | `src/physics/StringModel.h` | ✅ |
...
**預估工時**：4-5 天　→　**實際**：同日完成（與 Phase 1 合併作業）

### Phase 3：Cimbalom 引擎 — Modal Synthesis 方向 ✅ 完成（2026-05-08）
...
### Phase 4：引擎 — Chromatic Synth（物理建模升級） ✅ 完成（2026-05-08）
...
#### 架構更新
- PluginProcessor 重構為雙引擎架構（cimbalomSynth + chromaticSynth）
- 新增全域 engine 切換參數 + 7 個 Chromatic 專屬參數
- PluginEditor 引擎面板動態切換 UI
- Water Gong pitch glide（浸水效果模擬）

**預估工時**：4-5 天　→　**實際**：同日完成

### Phase 5：引擎 — FM Piano ✅ 完成（2026-05-08）
...
output = sin(carrier_freq × t + index × sin(modulator_freq × t + feedback × lastMod))
```

已完成：
- FM 合成核心（2-operator: carrier + modulator + self-feedback）
- Index envelope（指數衰減，獨立於 amplitude ADSR）
- 高音域的 brightness 衰減（note-dependent index decay）
- Velocity 雙重感應（gain + modulation index）
- 8 種 Sound Type preset（Piano/E.Piano/Vibraphone/Bell/Organ/Pad/Bass/Brass）
- 7 個 APVTS 參數（fm_type/ratio/index/brightness/feedback/attack/release）
- 三引擎架構完成（Cimbalom + Chromatic + FM Piano）

> 註：FM Piano 不屬於物理建模範疇，維持原始 FM 合成設計。

**預估工時**：4-5 天　→　**實際**：同日完成

### Phase 6：全域效果鏈 ✅ 完成（2026-05-08）
**目標：移植 Reverb、Delay、Wall Reflection**

| 效果 | 實作方式 | 狀態 |
|------|---------|------|
| Reverb | 自寫 Freeverb-style (8 comb + 4 allpass, stereo) | ✅ |
| Delay | StereoDelay (複用 DelayLine, LP in feedback) | ✅ |
| Wall Reflection | 整合至 Reverb early reflections | ✅ |
| Compressor | 自寫 Peak compressor (linked stereo, auto makeup) | ✅ |
| EffectChain | Comp → Delay → Reverb serial chain | ✅ |

7 個 FX APVTS 參數：reverb mix/size, delay time/feedback/mix, comp threshold/ratio
...
```
Phase 0   規劃              ████                                  ✅ 2026-05-06
Phase 1   環境+骨架          ░░░░████                              ✅ 2026-05-08
Phase 2   DSP+材質庫         ░░░░░░░░█████                         ✅ 2026-05-08
Phase 3   Cimbalom(物理建模)  ░░░░░░░░░░░░░██████                   ✅ 2026-05-08
Phase 4   Chromatic(物理建模) ░░░░░░░░░░░░░░░░░░██████                ✅ 2026-05-08
Phase 5   FM Piano           ░░░░░░░░░░░░░░░░░░░░░░░░██████          ✅ 2026-05-08
Phase 6   效果鏈             ░░░░░░░░░░░░░░░░░░░░░░░░░░░░██████    ✅ 2026-05-08
Phase 7   GUI                ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░████░░ ← 待設計
Phase 8   預設+收尾          ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░██████ ✅ 2026-05-08
─── VST 插件完成線 ──────────────────────────────────────────────
Phase 9   AAX(可選)          ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░██░░░░
...
### Windows（必要）
- [x] Visual Studio 2026 Community（`D:\Visual_Studio`，MSVC 14.50）
  - 已勾選「使用 C++ 的桌面開發」workload
- [x] CMake 4.3.2（winget 安裝）
- [x] Git（已有）

### 測試用 DAW
- [x] Cubase LE AI Elements 12（已驗證 VST3 載入 ✅）
...
1. 建立 `src/effects/` 目錄結構
2. Reverb（Schroeder 或 JUCE 內建 Freeverb）
3. Delay（複用 DelayLine + wet/dry mix）
4. Wall Reflection（3-tap delay + LP filter）
5. Compressor（limiter / dynamics control）
6. EffectChain 串接 + 全域 wet/dry 控制

**已達成里程碑：**
- ✅ 里程碑 1：在 DAW 裡載入空殼插件聽到正弦波（2026-05-08）
- ✅ 里程碑 2：正弦波替換為物理建模弦 → 聽到真實的揚琴泛音結構（2026-05-08）
- ✅ 里程碑 3：Chromatic 三合一引擎完成 — 梁/板/自訂泛音（2026-05-08）
- ✅ 里程碑 4：FM Piano 引擎 → 完成三引擎合一插件（2026-05-08）
- ✅ 里程碑 5：效果鏈完成 — Reverb/Delay/Compressor（2026-05-08）
- ⬜ 里程碑 6：GUI 設計（深色木質調介面 — 需月月參與）
