...
**當前目標：Phase 3 — Cimbalom 引擎（物理建模弦）**

1. 建立 `CimbalomEngine` class，接入 ModalResonator + StringModel
2. 用 MaterialDB 載入 materials.json，讓使用者切換材質
3. 替換現有正弦波 Voice → 物理建模弦
4. 加入擊打位置、弦徑、張力等參數控制
5. 加入多弦 beating（微失諧）+ 共鳴弦
6. 在 Cubase 中 A/B 對比原型 HTML 版

**已達成里程碑：**
- ✅ 里程碑 1：在 DAW 裡載入空殼插件聽到正弦波（2026-05-08）
- ⬜ 里程碑 2：正弦波替換為物理建模弦 → 聽到真實的揚琴泛音結構
